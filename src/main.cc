...
static void add_search_off_timeout(
    guint32 timeout,
    struct gigi* g,
    GtkFunction func = 0
)
...
static void run_with_system( const std::string& command, struct gigi* g )
{
    string cmd( command );
    cmd += '&';
...
// be there.
static void run_the_command( const std::string& command, struct gigi* g )
{
    string prog;
...
static void on_compline_runwithterm( GtkCompletionLine* cl, struct gigi* g )
{
    string command(
        g_locale_from_utf8(
            gtk_entry_get_text( GTK_ENTRY(cl) ),
            -1,
            NULL,
            NULL,
            NULL
...
static void on_search_mode( GtkCompletionLine* cl, struct gigi* g )
{
...
static void on_compline_activated( GtkCompletionLine* cl, struct gigi* g )
{
    ...
    string command = g_locale_from_utf8(
        gtk_entry_get_text( GTK_ENTRY(cl) ),
        -1,
        NULL,
        NULL,
    ...
    if( i != string::npos )
    {
        ...
        if( configuration.get_string_list("AlwaysInTerm", term_progs) )
        {
            ...
            cerr << "---" << std::endl;
#endif
            list<string>::const_iterator w = std::find(
                term_progs.begin(),
                term_progs.end(),
                progname
            );
 ...
 */
static gboolean screen_contains_pointer(
    GdkScreen* screen,
    int* x,
    int* y
)
...
/**
 ...
 Taked from Xfce: libxfcegui4
 */
static GdkScreen* gmrun_gdk_display_locate_monitor_with_pointer(
    GdkDisplay* display,
    gint* monitor_return
)
{
    ...
    for( i = 0; i < n_screens; i++ )
    {
        ...
        {
            if( monitor_return )
                *monitor_return = gdk_screen_get_monitor_at_point(
                    screen,
                    x,
                    y
                );
...
/**
 Center window on given monitor
  Taked from Xfce: libxfcegui4
 */
static void gmrun_gtk_window_place_on_monitor(
    GtkWindow* window,
    GdkScreen* screen,
    gint monitor,
    int request_x = -1,
...
static void gmrun_gtk_window_place_at(
    GtkWindow* window,
    int request_x,
    int request_y,
...
)
{
    ...
    if( on_active != 0 )
    {
        screen = gmrun_gdk_display_locate_monitor_with_pointer(
            NULL,
            &monitor
        );
    }
    ...
    gmrun_gtk_window_place_on_monitor(
        window,
        screen,
        monitor,
        request_x,
...
int main( int argc, char** argv )
{
    ...
    if( !configuration.get_int(
            "ShowDotFiles",
            GTK_COMPLETION_LINE( compline )->show_dot_files
        )
    )
    {
    ...
    gtk_signal_connect(
        GTK_OBJECT( compline ),
        "activate",
        GTK_SIGNAL_FUNC( on_compline_activated ),
        &g
    );
    gtk_signal_connect(
        GTK_OBJECT( compline ),
        "runwithterm",
        GTK_SIGNAL_FUNC( on_compline_runwithterm ),
        &g
    );
    gtk_signal_connect(
        GTK_OBJECT( compline ),
        "unique",
        GTK_SIGNAL_FUNC( on_compline_unique ),
        &g
    );
    gtk_signal_connect(
        GTK_OBJECT( compline ),
        "notunique",
        GTK_SIGNAL_FUNC( on_compline_notunique ),
        &g
    );
    gtk_signal_connect(
        GTK_OBJECT( compline ),
        "incomplete",
        GTK_SIGNAL_FUNC( on_compline_incomplete ),
        &g
    );
    gtk_signal_connect(
        GTK_OBJECT( compline ),
        "search_mode",
        GTK_SIGNAL_FUNC( on_search_mode ),
        &g
    );
    gtk_signal_connect(
        GTK_OBJECT( compline ),
        "search_not_found",
        GTK_SIGNAL_FUNC( on_search_not_found ),
        &g
    );
    gtk_signal_connect(
        GTK_OBJECT( compline ),
        "search_letter",
        GTK_SIGNAL_FUNC( on_search_letter ),
        label_search
    );
    gtk_signal_connect(
        GTK_OBJECT( compline ),
        "ext_handler",
        GTK_SIGNAL_FUNC( on_ext_handler ),
        &g
    );
    ...
    if( strcmp(geoptr, "") )
    {
        geo_parsed = gtk_window_parse_geometry( GTK_WINDOW(win), geoptr );
    }
    else
    {
        gmrun_gtk_window_place_at(
            GTK_WINDOW( win ),
            prefs_left,
            prefs_top,
