...
static void gtk_completion_line_class_init( GtkCompletionLineClass* klass )
{
    ...
    gtk_completion_line_signals[UNIQUE] = gtk_signal_new(
        "unique",
        GTK_RUN_FIRST,
        G_TYPE_FROM_CLASS( object_class ),
        GTK_SIGNAL_OFFSET( GtkCompletionLineClass, unique ),
        gtk_signal_default_marshaller,
        GTK_TYPE_NONE,
        0
    );
    gtk_completion_line_signals[NOTUNIQUE] = gtk_signal_new(
        "notunique",
        GTK_RUN_FIRST,
    G_TYPE_FROM_CLASS( object_class ),
        GTK_SIGNAL_OFFSET( GtkCompletionLineClass, notunique ),
        gtk_signal_default_marshaller,
        GTK_TYPE_NONE,
        0
    );
    gtk_completion_line_signals[INCOMPLETE] = gtk_signal_new(
        "incomplete",
        GTK_RUN_FIRST,
        G_TYPE_FROM_CLASS( object_class ),
        GTK_SIGNAL_OFFSET( GtkCompletionLineClass, incomplete ),
        gtk_signal_default_marshaller,
        GTK_TYPE_NONE,
        0
    );
    gtk_completion_line_signals[RUNWITHTERM] = gtk_signal_new(
        "runwithterm",
        GTK_RUN_FIRST,
        G_TYPE_FROM_CLASS( object_class ),
        GTK_SIGNAL_OFFSET( GtkCompletionLineClass, runwithterm ),
        gtk_signal_default_marshaller,
        GTK_TYPE_NONE,
        0
    );
    gtk_completion_line_signals[SEARCH_MODE] = gtk_signal_new(
        "search_mode",
        GTK_RUN_FIRST,
        G_TYPE_FROM_CLASS( object_class ),
        GTK_SIGNAL_OFFSET( GtkCompletionLineClass, search_mode ),
        gtk_signal_default_marshaller,
        GTK_TYPE_NONE,
        0
    );
    gtk_completion_line_signals[SEARCH_NOT_FOUND] = gtk_signal_new(
        "search_not_found",
        GTK_RUN_FIRST,
        G_TYPE_FROM_CLASS( object_class ),
        GTK_SIGNAL_OFFSET( GtkCompletionLineClass, search_not_found ),
        gtk_signal_default_marshaller,
        GTK_TYPE_NONE,
        0
    );
    gtk_completion_line_signals[SEARCH_LETTER] = gtk_signal_new(
        "search_letter",
        GTK_RUN_FIRST,
        G_TYPE_FROM_CLASS( object_class ),
        GTK_SIGNAL_OFFSET( GtkCompletionLineClass, search_letter ),
        gtk_signal_default_marshaller,
        GTK_TYPE_NONE,
        0
    );
    gtk_completion_line_signals[EXT_HANDLER] = gtk_signal_new(
        "ext_handler",
        GTK_RUN_FIRST,
        G_TYPE_FROM_CLASS( object_class ),
        GTK_SIGNAL_OFFSET( GtkCompletionLineClass, ext_handler ),
        gtk_marshal_NONE__POINTER,
        GTK_TYPE_NONE,
        1,
        GTK_TYPE_POINTER
    );
    gtk_completion_line_signals[CANCEL] = gtk_signal_new(
        "cancel",
        GTK_RUN_FIRST,
        G_TYPE_FROM_CLASS( object_class ),
        GTK_SIGNAL_OFFSET( GtkCompletionLineClass, ext_handler ),
...
static void gtk_completion_line_init( GtkCompletionLine* object )
{
    ...
    on_key_press_handler = gtk_signal_connect(
        GTK_OBJECT( object ),
        "key_press_event",
...
)
{
    ...
    vector<string>::const_iterator i_end = words.end();
...
static void complete_from_list( GtkCompletionLine* object )
{
    ...
    if( object->win_compl != NULL )
    {
        object->where = (GList*)gtk_clist_get_row_data(
            GTK_CLIST( object->list_compl ),
            object->list_compl_items_where
        );
...
/*
static void select_appropiate( GtkCompletionLine* object )
{
...
static void get_prefix( GtkCompletionLine* object )
{
    parse_tilda( object );
    vector<string> words;
    int pos = get_words( object, words );
        ...
        if( words[pos] == ((GString*)ls->data)->str )
        {
            if( object->win_compl == NULL )
            {
                ...
                */
                ...
                on_row_selected_handler = gtk_signal_connect(
                    GTK_OBJECT( object->list_compl ),
                    "select_row",
                ...
                gtk_container_set_border_width(
                    GTK_CONTAINER( object->list_compl ),
                    2
                );
                gtk_container_add(
                ...
                int w = gtk_clist_optimal_column_width(
                    GTK_CLIST( object->list_compl ),
                    0
                );
                ...
                GdkWindow* top = gtk_widget_get_parent_window(
                    GTK_WIDGET( object )
                );
...
static int search_back_history( GtkCompletionLine* cl, bool avance, bool begin )
{
...
static int search_forward_history(
    GtkCompletionLine* cl,
    bool avance,
    bool begin
)
...
/*
static int inverse_search_history(
    GtkCompletionLine* cl,
    bool avance,
    bool begin
)
        ...
        default:
            ...
            if( cl->tabtimeout != 0 )
            {
                ...
                if( ::isprint( *event->string ) )
                    tt_id = gtk_timeout_add(
                        cl->tabtimeout,
                        GtkFunction( tab_pressed ),
                        cl
                    );
            }
